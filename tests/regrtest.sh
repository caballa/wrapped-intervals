...
echo "Running t1.c"
$CMMD $TEST_DIR/t1.c $PASS -widening 3 -narrowing 1 >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t2.c"
$CMMD $TEST_DIR/t2.c $PASS -widening 3 -narrowing 1 -inline 300 >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t3.c"
$CMMD $TEST_DIR/t3.c $PASS -widening 3 -narrowing 1 >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t4.c"
$CMMD $TEST_DIR/t4.c $PASS -widening 3 -narrowing 1 -instcombine >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t5.c"
$CMMD $TEST_DIR/t5.c $PASS -widening 3 -narrowing 1 -instcombine -inline 300 >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t6.c"
$CMMD $TEST_DIR/t6.c $PASS -widening 3 -narrowing 1 -inline 300 >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t7.c"
$CMMD $TEST_DIR/t7.c $PASS -widening 3 -narrowing 1 -inline 300 >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t8.c"
$CMMD $TEST_DIR/t8.c $PASS -widening 3 -narrowing 1 >& $TEST_DIR/log 
getAndCheckStats $TEST_DIR/log 0 0
echo "Running t9.c"
$CMMD $TEST_DIR/t9.c $PASS -widening 4 -narrowing 1 >& $TEST_DIR/log
getAndCheckStats $TEST_DIR/log 0 0
